...
publisher = rospy.Publisher("motor_status", SmartServoStatus, queue_size=10)
...
def init():
    ...
    for j,b in rospy.get_param('/joints').items():
        ...
        busname = '/servobus/' + str(number).zfill(2) + '/smartservostatus'

        # and if it's not already in the bus{}, then add it
        # (not sure if the check is required)
        if not bus.has_key(number):
            bus[number] = rospy.Subscriber(busname, SmartServoStatus, dispatcher, (number))
