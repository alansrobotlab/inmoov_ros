...
class Pub3DTarget():
    def __init__(self):
        ...
        marker_pub = rospy.Publisher('target_point_marker', Marker, queue_size=10)

        #clicked_sub = rospy.Subscriber('clicked_point', PointStamped, self.clicked_callback)
        ...
        self.target.header.frame_id = target_frame
        ...
        while not rospy.is_shutdown():                   
            ...
            self.target.point.x = (row*2) + 5
            self.target.point.y = (seat-5) * 1.0
            self.target.point.z = 1.35
        
            self.target.header.stamp = rospy.Time.now()
            self.target_pub.publish(self.target)

            rospy.loginfo("Publishing target point" + str(self.target))
    ...
    def clicked_callback(self, data):
        ...
        self.target.point.x = data.point.y
        self.target.point.y = -data.point.x
        self.target.point.z = data.point.z - 1.35
